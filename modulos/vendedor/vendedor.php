...
function comprueba_si_existe_codigo_vendedor($codigo_vendedor)
{
    global $base_de_datos;
    $sentencia = $base_de_datos->prepare("SELECT count(*) AS count FROM vendedor WHERE codigo_vendedor = ?;");
    $sentencia->execute([$codigo_vendedor]);
    $fila = $sentencia->fetch();
    $numero_filas = $fila["count"];
    if ($numero_filas >= 1) {
        $sentencia = $base_de_datos->prepare("SELECT * FROM vendedor WHERE codigo_vendedor = ?;");
        $sentencia->execute([$codigo_vendedor]);
...
{
    global $base_de_datos;
    $sentencia = $base_de_datos->prepare("SELECT * FROM vendedor WHERE codigo_vendedor LIKE ? OR nombre LIKE ? LIMIT 10;");
